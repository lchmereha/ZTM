...
class TransferenciaController extends GetxController {
  ...
  Future<void> fetchProducts() async {
    ...
    try {
      final list = await _movApi.getTransferenciaProdutos(movementId);
      products.assignAll(
        list.map((item) {
      ...
      Get.dialog(
        ErrorDialog(
          message: 'Falha ao buscar produtos da transferência.',
          detalhes: e.toString(),
        ),
      );
    } finally {
  ...
  Future<void> submit() async {
    ...
    if (hasDivergence) {
      final confirm = await Get.dialog<bool>(
        const BooleanDialog(
          ...
          content:
              'Existem diferenças entre a quantidade conferida e a esperada. Deseja finalizar a transferência mesmo assim?',
          trueLabel: 'Finalizar',
          falseLabel: 'Cancelar',
        ),
      );
      ...
      final confirm = await Get.dialog<bool>(
        const BooleanDialog(
          ...
          content:
              'Todas as quantidades conferem. Deseja concluir a transferência no servidor?',
        ),
      );
      if (confirm != true) return;
    }
    ...
    Get.dialog(
      const LoadingDialog(
        title: 'Enviando',
        content: 'Concluindo transferência no servidor...',
      ),
    ...
    try {
      await _movApi.concluirTransferencia(movementId, vinculacoes);
      if (Get.isDialogOpen == true) Get.back(); // close loading dialog
      Get.back(); // return to dashboard
      _snackbar.success('Sucesso', 'Transferência concluída com sucesso.');
    } on ApiException catch (e) {
      ...
      Get.dialog(
        ErrorDialog(
          message: 'Falha ao concluir transferência no servidor.',
          detalhes: e.toString(),
        ),
      );
    } finally {

...
interface ProdutoTransferencia {
...
interface TransferenciaLeituraProps {
...
const TransferenciaLeitura = forwardRef<MovimentacaoStepHandler, TransferenciaLeituraProps>(
  ({ movimentacaoId, onComplete, onStateChange, onSituacaoChange, finalizado = false }, ref) => {
    ...
    const activeTagMap = useMemo(() => {
      const map = new Map<string, { tagAtiva: TagAtiva; produto: ProdutoTransferencia }>();
    ...
    // Total tags needed = sum of quantidadeTransferencia across all products
    const totalNeeded = useMemo(() => produtos.reduce((acc, p) => acc + p.quantidadeTransferencia, 0), [produtos]);
    ...
    // already has enough matches (read more tags than quantidadeTransferencia)
    ...
    useEffect(() => {
      ...
      setVinculacoes((prev) => {
        ...
        for (const readTag of tags) {
          ...
          // Only match if product still has room (count < quantidadeTransferencia)
          const currentCount = countPerProduct.get(produto.idProduto) ?? 0;
          if (currentCount >= produto.quantidadeTransferencia) continue;
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          // Fetch products for transferencia
          const { data: prodData } = await axios.get(`${movimentacaoEndpoint}/${movimentacaoId}/transferencia/produtos`);
        ...
        actions: [
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                // Direct post for Transferencia
                const { data } = await axios.post(`${movimentacaoEndpoint}/${movimentacaoId}/transferencia/concluir`, {
                  vinculacoes: vinculacoesList
                });

                showSnackbar({
                  title: 'Transferência concluída!',
                  message: `${data.totalConferidas ?? vinculacoesList.length} ${(data.totalConferidas ?? vinculacoesList.length) === 1 ? 'tag foi transferida' : 'tags foram transferidas'} com sucesso.`,
                  severity: 'success'
                });
      ...
      <Box>
        ...
        <Box>
          {produtos.map((produto) => {
            ...
            const isFull = confirmedCount >= produto.quantidadeTransferencia;
              ...
              >
                <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                  <Stack direction="row" spacing={1} sx={{ alignItems: 'center', width: '100%', mr: 2 }}>
                    ...
                    <Chip
                      label={`${confirmedCount} / ${produto.quantidadeTransferencia}`}
                      size="small"
...
export default TransferenciaLeitura;

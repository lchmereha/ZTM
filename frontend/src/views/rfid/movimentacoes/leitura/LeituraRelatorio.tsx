...
interface LeituraRelatorioProps {
  movimentacaoId: number;
  onStateChange?: () => void;
}
...
const LeituraRelatorio = forwardRef<LeituraHandler, LeituraRelatorioProps>(({ movimentacaoId, onStateChange }, ref) => {
  ...
  useEffect(() => {
    const fetchRelatorio = async () => {
      try {
        const { data } = await axios.get<RelatorioResponse>(`${movimentacaoEndpoint}/${movimentacaoId}/leitura/relatorio`);
        setRelatorio(data);
      } catch (err) {
        handleError(err);
      } finally {
    ...
    return (
      ...
      >
        <Typography variant="h6" sx={{ color: 'text.secondary' }}>
          Nenhuma etiqueta baixada encontrada
        </Typography>
        <Typography variant="body2" sx={{ color: 'text.disabled' }}>
          Esta movimentação não possui etiquetas com baixa registrada.
        </Typography>
      </Paper>
    );
  }
  ...
  return (
    <Box>
      <Stack direction="row" spacing={1} sx={{ mb: 2, alignItems: 'center' }}>
        <Typography variant="h6">Relatório de Baixa</Typography>
        <Chip
          label={`${relatorio.totalTags} ${relatorio.totalTags === 1 ? 'etiqueta' : 'etiquetas'}`}
          color="success"
          size="small"
          variant="filled"
        />
        ...
        >
          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
            <Stack direction="row" spacing={1} sx={{ alignItems: 'center', width: '100%' }}>
              ...
              <Chip
                label={`${produto.tags.length} ${produto.tags.length === 1 ? 'etiqueta' : 'etiquetas'}`}
                size="small"
                variant="outlined"
                sx={{ ml: 'auto' }}
              />
                  ...
                  <ListItemText
                    ...
                    secondary={
                      [
                        ...
                        tag.dataBaixa && `Baixa: ${new Date(tag.dataBaixa).toLocaleDateString('pt-BR')}`,
...
export default LeituraRelatorio;
